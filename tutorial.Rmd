...
library(dplyr)
s_data <- pData(gse) %>% 
  select(geo_accession, contains("characteristics"), -characteristics_ch1.2, -characteristics_ch1, -characteristics_ch1.1)
```
...
```{r}
s_data <- s_data %>% 
  rename(hospital = characteristics_ch1.3,
...
```{r}
features <- select(features, ID, `Gene Symbol`)
```


We can also notice that some rows have multiple entries separated by the `///` set of characters. We can split into multiple columns using the `separate` function from `tidyr`.
...
Now that we have gene symbols in the same table as the expression data we can search according to the gene name. 

```{r}
e_data <- filter(e_data,Symbol_1 %in% "ESR1") %>% 
  select(-Symbol_1, -Symbol_2)
```
...
We can now join these expression data to our `s_data` data frame that we are using for survival analysis. This is possible because both `s_data` and `e_data` have a column (`geo_accession`) in common.
...
As we saw before, the survival analysis requires a grouping variable to split our data (previously we used the `er` column). We can form the groups using the expression from one particular probe
...
```{r}
s_data <- mutate(s_data, Group = cut(`205225_at`, breaks = quantile(`205225_at`,probs=c(0,0.25,0.5,1))))
```
